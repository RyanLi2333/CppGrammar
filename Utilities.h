...
文件后缀作为文件名的最后一部分，一来是用于区分不同的文件(不含后缀的同名不同类型文件)，
二来是提供一种解析建议，操作系统根据默认应用与这些后缀的关联，来默认地解析这些后缀的文件(当然也可以手动指定其他解析方式)。
文件名(含后缀)在某个文件夹，统一级别目录下是唯一标识的。
而决定文件是否可以被某种方式解读，取决于该文件的实质内容(二进制or文本)
*/
